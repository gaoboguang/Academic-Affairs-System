...
class StudentPathwayProfile(PrimaryKeyMixin, TimestampMixin, ActiveMixin, Base):
    ...
    art_professional_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    art_professional_full_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    art_score_source: Mapped[str | None] = mapped_column(String(150), nullable=True)
    art_score_note: Mapped[str | None] = mapped_column(Text, nullable=True)
