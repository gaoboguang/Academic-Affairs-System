<template>
  <section class="soft-card panel-block">
    ...
    <el-alert v-if="loadError" class="rule-panel-alert" type="error" show-icon :closable="false" title="赋分规则加载失败">
      <template #default>
        <div class="rule-alert-body">
          <span>{{ loadError }}</span>
          <el-button link type="primary" :loading="loading" @click="emit('load')">重新加载赋分规则</el-button>
        </div>
      </template>
    </el-alert>
    ...
    <div class="filter-grid">
      ...
      <el-input v-model="filters.subject_name" clearable :disabled="loading" placeholder="科目" />
    </div>
...
const hasActiveFilters = computed(() =>
  Boolean(props.filters.year || props.filters.province || props.filters.exam_mode || props.filters.subject_name),
);
const ruleEmptyDescription = computed(() => {
  if (props.loadError) return "赋分/成绩转换规则加载失败，请重新加载。";
  if (props.loading) return "正在加载赋分/成绩转换规则";
  if (hasActiveFilters.value) return "没有符合当前筛选条件的赋分规则。";
  return "暂无赋分/成绩转换规则，可以先装载基线。";
});
