import {
  ...
  classKnowledgeBriefingPrintPreviewPath,
  ...
  studentKnowledgePrintPreviewPath,
...
export const REPORT_TYPE_OPTIONS: Array<{ value: string; label: string }> = [
  ...
  { value: "class_knowledge_briefing", label: "班级知识点讲评清单" },
...
const REPORT_TYPE_CONFIGS: Record<string, ReportTypeConfig> = {
  ...
  student_knowledge_plan: {
    ...
    requiredFields: ["exam_id", "student_id"],
    buildPrintPreviewPath: (form) =>
      form.student_id && form.exam_id ? studentKnowledgePrintPreviewPath(form.student_id, form.exam_id) : null,
  },
  class_knowledge_briefing: {
    value: "class_knowledge_briefing",
    label: "班级知识点讲评清单",
    domain: "scores",
    purpose: "给班主任或备课组输出班级共性薄弱知识点、错因分布和讲评任务。",
    dataSources: ["题分明细", "知识点诊断", "班级名单", "错因标签"],
    formats: ["Excel", "打印预览"],
    riskTags: ["需导入题分明细", "需选择考试", "需选择班级"],
    requiredFields: ["exam_id", "class_id"],
    buildPrintPreviewPath: (form) =>
      form.class_id && form.exam_id ? classKnowledgeBriefingPrintPreviewPath(form.class_id, form.exam_id) : null,
  },
  class_analysis: {
  ...
  adviser_weekly_summary: {
    ...
    label: "班主任周报",
    domain: "adviser",
    purpose: "用于给班主任或年级组输出班级成绩波动、成长档案、规划任务和重点学生。",
    dataSources: ["班级名单", "考试成绩", "成长档案", "升学规划任务"],
    formats: ["Excel", "打印预览"],
    riskTags: ["需选择班级", "缺成绩样本时标明数据缺口", "保留生成时间"],
  ...
  student_followup_package: {
    ...
    label: "学生跟进包",
    domain: "adviser",
    purpose: "用于汇总单个学生的成绩趋势、成长记录、规划任务和建议行动。",
    dataSources: ["学生档案", "考试成绩", "成长档案", "升学规划任务", "升学画像"],
    formats: ["Excel", "打印预览"],
  ...
  planning_followup: {
    ...
    dataSources: ["升学规划", "升学画像", "路径初筛", "成绩与成长跟进"],
    formats: ["Excel", "打印预览"],
