<template>
  <div class="print-shell">
    ...
    <div v-if="loading" class="print-placeholder">正在加载班级分析报表...</div>
    ...
    <article v-else-if="analytics" class="print-page">
      <header class="print-header">
        <div>
          <div class="print-eyebrow">班级成绩分析报表 / 打印预览</div>
          <h1>{{ analytics.class_name }}</h1>
          <p>{{ analytics.exam_name }}</p>
        </div>
        <div class="print-summary-grid">
          <div class="print-summary-item"><span>班级人数</span><strong>{{ analytics.student_count }}</strong></div>
          <div class="print-summary-item"><span>总分均分</span><strong>{{ analytics.total_average }}</strong></div>
          <div class="print-summary-item"><span>总分中位数</span><strong>{{ analytics.total_median }}</strong></div>
          <div class="print-summary-item"><span>年级均分</span><strong>{{ analytics.grade_average ?? "-" }}</strong></div>
        </div>
      </header>
      ...
      <section class="print-section">
        <div class="print-section-head"><h2>学科表现</h2><span>{{ analytics.subject_breakdown.length }} 科</span></div>
        <table class="print-table">
          <thead><tr><th>科目</th><th>均分</th><th>中位数</th><th>最高分</th><th>最低分</th><th>标准差</th><th>优秀率</th><th>及格率</th></tr></thead>
          <tbody>
            <tr v-for="item in analytics.subject_breakdown" :key="item.subject_id">
              <td>{{ item.subject_name }}</td><td>{{ item.average_score }}</td><td>{{ item.median_score }}</td><td>{{ item.max_score }}</td><td>{{ item.min_score }}</td><td>{{ item.standard_deviation }}</td><td>{{ item.excellent_rate }}</td><td>{{ item.pass_rate }}</td>
            </tr>
          </tbody>
        </table>
      </section>
...
import { buildClassAnalysisInsightCards } from "../components/reports/reportInsights";

interface SubjectAggregateItem { subject_id: number; subject_name: string; average_score: number; median_score: number; max_score: number; min_score: number; standard_deviation: number; excellent_rate: number; pass_rate: number; }
interface ClassAnalyticsResponse { exam_name: string; class_name: string; student_count: number; total_average: number; total_median: number; grade_average: number | null; subject_breakdown: SubjectAggregateItem[]; }

const route = useRoute();
const loading = ref(true);
const errorMessage = ref("");
const analytics = ref<ClassAnalyticsResponse | null>(null);
const insightCards = computed(() => (analytics.value ? buildClassAnalysisInsightCards(analytics.value) : []));
...
async function loadPrintData(): Promise<void> {
  ...
  analytics.value = await apiRequest<ClassAnalyticsResponse>(`/api/analytics/classes/${classId}?exam_id=${examId}`);
}
