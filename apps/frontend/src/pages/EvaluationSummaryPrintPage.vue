<template>
  <div class="print-shell">
    ...
    <div v-if="loading" class="print-placeholder">正在加载评教汇总...</div>
    <div v-else-if="errorMessage" class="print-placeholder danger">{{ errorMessage }}</div>
    <article v-else-if="overview" class="print-page">
      <header class="print-header">
        <div>
          <div class="print-eyebrow">评教汇总报表 / 打印预览</div>
          <h1>{{ overview.template_name }}</h1>
          <p>{{ overview.semester_name ?? "未标注学期" }}</p>
        </div>
        <div class="print-summary-grid">
          <div class="print-summary-item"><span>教师数</span><strong>{{ overview.teacher_count }}</strong></div>
          <div class="print-summary-item"><span>批次 ID</span><strong>{{ overview.batch_id }}</strong></div>
        </div>
      </header>
      ...
      <section class="print-section">
        <div class="print-section-head"><h2>教师汇总</h2><span>{{ overview.teacher_summaries.length }} 条</span></div>
        <table class="print-table">
          <thead><tr><th>教师</th><th>综合均分</th><th>响应数</th><th>排名</th><th>维度得分</th></tr></thead>
          <tbody>
            <tr v-for="item in overview.teacher_summaries" :key="item.teacher_id">
              <td>{{ item.teacher_name }}</td><td>{{ item.overall_avg_score }}</td><td>{{ item.response_count }}</td><td>{{ item.rank ?? "-" }}</td><td>{{ formatDimensionScores(item.dimension_scores_json) }}</td>
            </tr>
          </tbody>
        </table>
      </section>
...
import { buildEvaluationInsightCards } from "../components/reports/reportInsights";

interface EvaluationTeacherSummaryRead { teacher_id: number; teacher_name: string; overall_avg_score: number; response_count: number; rank: number | null; dimension_scores_json?: Record<string, number> | null; }
interface EvaluationBatchOverviewRead { batch_id: number; template_name: string; semester_name?: string | null; teacher_count: number; teacher_summaries: EvaluationTeacherSummaryRead[]; }

const route = useRoute();
const loading = ref(true);
const errorMessage = ref("");
const overview = ref<EvaluationBatchOverviewRead | null>(null);
const insightCards = computed(() => (overview.value ? buildEvaluationInsightCards(overview.value) : []));

function goBack(): void { window.history.back(); }
function printPage(): void { window.print(); }
function formatDimensionScores(value?: Record<string, number> | null): string {
  if (!value) return "-";
...
async function loadPrintData(): Promise<void> {
  ...
  if (!batchId) { errorMessage.value = "打印参数无效"; return; }
  overview.value = await apiRequest<EvaluationBatchOverviewRead>(`/api/evaluation/batches/${batchId}/overview`);
}
