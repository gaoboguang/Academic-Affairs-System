...
- 文档版本：v1.0
- 文档日期：2026-04-03
- 目标形态：本地单机运行、仅个人使用、不上传公网服务器、不依赖多用户协作
...
本项目不是“全校在线协同教务平台”，而是一个**本地、单用户、数据驱动的教务决策台**。
...
5. 所有敏感数据默认本地保存，不连接公网服务，不依赖云数据库。
