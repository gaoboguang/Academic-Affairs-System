...
- `tests/e2e`：当前已落地 Playwright 跨端流程，默认启动临时演示数据后端和 Vite 前端；E2E 已按业务域拆成 dashboard、students、exams-analytics、reports、recommendations、gaokao-volunteer、system-backup 7 个 spec，覆盖工作台导航、学生详情、系统备份、考试到分析、推荐生成/导出、多方案对比、策略模板、历史回放、Stage B 主链路、批量混合生源地、缺少年份规则解释、模式兼容回退、高考志愿主流程、报表与打印预览，以及多条异常提示回归
...
- `npm run check:e2e`：运行 `tests/e2e` 下的分域 Playwright 跨端流程。
